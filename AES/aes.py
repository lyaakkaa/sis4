...
class AES:
    # Создается словарь rounds_by_key_size, который содержит количество раундов AES в зависимости от размера ключа. Например, если размер ключа 16 байт, то количество раундов будет 10.
    rounds_by_key_size = {16: 10, 24: 12, 32: 14}
    def __init__(self, master_key):
        # Проверяется, что длина master_key соответствует одному из размеров ключей, определенных в rounds_by_key_size. Если длина ключа не соответствует ни одному из этих размеров, будет вызвано исключение.
        assert len(master_key) in AES.rounds_by_key_size
        self.n_rounds = AES.rounds_by_key_size[len(master_key)]
        self._key_matrices = self._expand_key(master_key)
    # Определяется метод _expand_key, который расширяет исходный ключ до формы, подходящей для использования в алгоритме AES. Он принимает master_key.
    def _expand_key(self, master_key):
        key_columns = bytes2matrix(master_key)
        iteration_size = len(master_key) // 4
        i = 1
        # Начинается цикл, который продолжается до тех пор, пока количество столбцов в матрице ключа не станет равным необходимому для всех раундов AES.
        while len(key_columns) < (self.n_rounds + 1) * 4:
            word = list(key_columns[-1])
            # Проверяется, является ли текущий столбец началом новой итерации.
    ...
    # Эта функция предназначена для шифрования одного блока данных (размером 16 байт) с использованием алгоритма AES.
    def encrypt_block(self, plaintext):
        #  Эта строка проверяет, что длина переданного блока данных (plaintext) составляет 16 байт. Это требование AES, который оперирует с блоками размером 128 бит (16 байт)
